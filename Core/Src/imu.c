...
uint8_t readIMU (uint8_t readReg){
	...
	HAL_GPIO_WritePin(IMU_NCS_GPIO_Port, IMU_NCS_Pin, GPIO_PIN_RESET); //pulling low methinks
	HAL_SPI_TransmitReceive(&hspi1, txData, rxData, 2, HAL_MAX_DELAY);
	HAL_GPIO_WritePin(IMU_NCS_GPIO_Port, IMU_NCS_Pin, GPIO_PIN_SET); //pulling high
...
void configIMU(void){
	writeIMU(0x4E, 0x03); // power management register
	writeIMU(0x50, 0x02); // accelerometer configuration register
}
